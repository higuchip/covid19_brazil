...
summary(m)
m
my.eq<-"8.2361 - 0.5817*x^2 + 0.0548*x^3"
p<-ggplot(data=covid19, aes(t,casos)) +
...
label.text <- paste(gsub("x", "~italic(x)", my.eq, fixed = TRUE),
                    paste("italic(R)^2",
                          ...
                          sep = "~`=`~"),
                    sep = "~~~~")
p<-p+labs(y="Casos confirmados de COVID-19", x = "Dias desde o primeiro caso registrado de COVID-19", caption = "Fonte: Ministério da Saúde (Dados atualizados em 15/03/2020 às 19:15)") +
...
p<-p + annotate(geom = "text", x = 0, y = 150, label = label.text,
...
p+annotate(geom = "curve", x = 19, y = 250, xend = 19, yend = 200, 
  curvature = 0, arrow = arrow(length = unit(2, "mm"))) +
  annotate(geom = "text", x = 17, y = 255, label = "15/03/20", hjust = "left", cex=6)
